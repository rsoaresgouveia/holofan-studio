...
### ✅ UPLOAD SOLVED — validated end-to-end on real hardware (2026-07-22)

The full sequence, confirmed by uploading a clip that then appeared in the playlist:

```
fresh TCP connection (no handshake)
send  BEGIN  = "B2DDDDEDC0EEBDF9E5B7"   (20 B)
send  filename chunk  "B2DDDDED" + len(3) + name + "C0EEBDF9E5B7"
recv  ack             (28 B, "…0b <token>…")
send  raw file bytes  (0x3c000-byte blocks; NOT framed; no per-block ack)
send  END    = "B2DDDDEDC0EEBDF9E5B7"   (20 B)
close
```

**The one non-obvious rule: the filename MUST end in `.BIN`.** With a bare name the device acks it
but silently discards the data; with `NAME.BIN` it commits and lists the clip as `NAME` (extension
stripped). Everything else was faithful all along — the "no reconnect" reading (the `0x40d0d4` calls
are `closesocket`, not `connect`) and the raw un-framed data were both correct. The ack's bytes 12–15
are a client-side filename-integrity token (`[edi+0x44b0]`); they don't gate the device.

Implemented in `FanClient.UploadAsync` (appends `.BIN`) + `POST /api/fan/upload` + the "Send to fan"
button. `ChunkLengthBytes` is unit-tested against device-verified values.
